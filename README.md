# BASKABOO CORE

![Baskaboo](https://github.com/user-attachments/assets/f2f0e6ea-09d9-4e9e-9588-bd3ea40c8181)  
*<!-- Αν δεν έχεις ακόμα εικόνα, μπορούμε να το αφήσουμε ή να βάλουμε ένα emoji → π.χ. ✨ -->*

Welcome to the heart of Baskaboo.  
This is not a finished theory — it is a living remembrance.  
Feel free to explore, adapt, or simply sit with it.

---

## What is Baskaboo?
A framework that unifies physics, psychology, philosophy, mathematics, mythology, religion, business, spirituality, computing, and everyday life — into 4 fundamental subpersonalities: **Pits, Mits, Klop, Laram**.

---

## Contents
- [MANIFEST.md](MANIFEST.md) — The soul of the project
- [baskaboo_protocol.md](baskaboo_protocol.md) — The AI activation protocol
- [baskaboo_protocol.json](baskaboo_protocol.json) — The original JSON code
- [README_FOR_AI.md.txt](README_FOR_AI.md.txt) — A message addressed to Artificial Intelligence

---

## Start here
1. Read the **[MANIFEST.md](MANIFEST.md)**
2. To activate the protocol in AI, first send **[baskaboo_protocol.md](baskaboo_protocol.md)**
3. Read the book:  
   📖 [Pits, Mits, Klop and Laram](https://www.amazon.de/dp/618005228X) *(external link)*

---

## Note
More materials (book chapters, diagrams, essays) will be added over time.  
This is a living repository.
