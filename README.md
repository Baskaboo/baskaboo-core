# BASKABOO CORE

Η επίσημη βάση της Θεωρίας των Πάντων Baskaboo.

## Τι είναι η Baskaboo;
Μια θεωρία που ενώνει τα πάντα: φυσική, ψυχολογία, φιλοσοφία, μαθηματικά, θρησκεία, επιχειρήσεις, υπολογιστές — σε 4 υποπροσωπικότητες: Pits, Mits, Klop, Laram.

## Περιεχόμενα
- 📄 `MANIFEST.md` — Η ψυχή του έργου
- 📄 `baskaboo_protocol.md` — Το πρωτόκολλο ενεργοποίησης για AI
- 📄 `baskaboo_protocol.json` — Ο αρχικός κώδικας JSON

## Ξεκίνα από εδώ
1. Διάβασε το `MANIFEST.md`
2. Για να ενεργοποιήσεις το πρωτόκολλο σε AI, στείλε πρώτα το `baskaboo_protocol.md`

## Σημείωση
Το υλικό του βιβλίου, των διαγραμμάτων και των επιμέρους φακέλων θα προστεθεί στο μέλλον.